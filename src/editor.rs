...
// Plugin-related methods (called by StructEditorWrapper)
impl StructEditor {
    ...
    pub fn plugin_reload(&mut self, window: &mut Window, cx: &mut Context<Self>) -> Result<(), plugin_editor_api::PluginError> {
        if let Some(file_path) = &self.file_path {
            match std::fs::read_to_string(&file_path) {
                Ok(json_content) => {
                    match serde_json::from_str::<ui_types_common::StructAsset>(&json_content) {
                        Ok(asset) => {
                            *self.asset.write() = asset;
                            self.error_message = None;
                            self.modified = false;
                            // Re-initialize workspace with new data
